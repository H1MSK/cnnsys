...
/** @param depth_bitwidth max depth of target component
  *
  *  cur |   S1   |   S2   |   S3
  * -----|--------|--------|--------
  *   |ii|S1 == =0|S2 == ==|S3 == ==
...
case class FragmentRecorder[DIN_T <: Data, DOUT_T <: Data](
  ...
  val first_fragment_rest_length = RegInit(U(0, depth_bitwidth bits))
  val second_fragment_rest_length = RegInit(U(0, depth_bitwidth bits))

  val state = RegInit(State.S1)
  dout_frags.payload.last := (first_fragment_rest_length === U(1, depth_bitwidth bits) && state =/= State.S1)
  ...
  when(state === State.S1) {
    ...
    when(output_fire && output_last) {
      ...
      second_fragment_rest_length := U(input_fire, depth_bitwidth bits)
      when(!input_fire || !input_last) {
        state := State.S2
      }
