      ...
      input_widths_options.foreach(input_widths => {
        new TestTask[Input2DPadder[SInt]](included) {
          ...
          override def doSim(compiled: SimCompiled[Input2DPadder[SInt]]): Unit = {
            compiled.doSim("Input2DPadder_")(dut => {
              ...
              (0 to max_padding_size).foreach(padding_size => {
                ...
                input_widths.indices.foreach(sel => {

                  val padding_data = TestTask.randomSInt(8 bits)

                  val width = input_widths(sel)
